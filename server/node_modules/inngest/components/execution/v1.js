...
import { createDeferredPromise, createDeferredPromiseWithStack, createTimeoutPromise, resolveAfterPending, runAsPromise } from "../../helpers/promises.js";
	...
	*/
	...
	constructor(options) {
		...
		*/
		...
		this.checkpointHandlers = this.options.stepMode === StepMode.Sync ? this.createCheckpointingCheckpointHandlers() : this.createCheckpointHandlers();
		...
		return {
			...
			"steps-found": async ({ steps }) => {
				...
				if (result.error) {
					await this.checkpoint([{
						...step,
						op: step.op === StepOpCode.StepPlanned ? StepOpCode.StepError : step.op,
						error: result.error
					}]);
				...
				if ("data" in result) {
					...
					await this.checkpoint([{
						id: step.hashedId,
						data: step.data,
						op: step.op === StepOpCode.StepPlanned ? StepOpCode.StepRun : step.op,
