	...
	*/
	...
	constructor(options) {
		...
		*/
		...
		this.checkpointHandlers = this.options.stepMode === require_types.StepMode.Sync ? this.createCheckpointingCheckpointHandlers() : this.createCheckpointHandlers();
	...
	createCheckpointingCheckpointHandlers() {
		return {
			...
			"steps-found": async ({ steps }) => {
				if (steps.length !== 1 || steps[0].mode !== require_types.StepMode.Sync) return this.checkpointAndSwitchToAsync(steps.map((step$1) => ({
					...step$1,
					id: step$1.hashedId
				})));
				...
				if (result.error) {
					await this.checkpoint([{
						...step,
						op: step.op === require_types.StepOpCode.StepPlanned ? require_types.StepOpCode.StepError : step.op,
						error: result.error
					}]);
				...
				if ("data" in result) {
					...
					await this.checkpoint([{
						id: step.hashedId,
						data: step.data,
						op: step.op === require_types.StepOpCode.StepPlanned ? require_types.StepOpCode.StepRun : step.op,
