...
const ChatBox = () => {
  ...
  }, [currentUser?._id, userId, dispatch])
    ...
    <div className='flex flex-col h-screen'>
      ...
      <div className='p-5 md:px-10 h-full overflow-y-scroll'>
        <div className='space-y-4 max-w-4xl mx-auto'>
          {messages.map((message, index) => (
            <div key={index} className={`flex flex-col ${message.to_user_id === userId ? 'items-end' : 'items-start'}`}>
              <div className={`p-2 text-sm max-w-sm bg-white text-slate-700 rounded-lg shadow ${message.to_user_id === userId ? 'rounded-br-none' : 'rounded-bl-none'}`}>
      ...
      <div className='px-4'>
        <div className='flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5'>
          ...
          <label htmlFor="image">
            {image ? <img src={URL.createObjectURL(image)} alt='' className='h-8 rounded' /> : <ImageIcon className='size-7 text-gray-700 cursor-pointer' />}
